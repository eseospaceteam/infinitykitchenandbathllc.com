...
//
// Fourteen rules, all deliberately DIFFERENT from the heuristic spam scoring used
...
const RANKING_TOPIC =
  /\b(?:first|1st)\s+page\s+of\s+google\b|\bpage\s*(?:one|1)\s+of\s+google\b|\btop\s+of\s+google\b|\btop\s+(?:10|ten|5|five)\s+(?:on|in)\s+google\b|\bnot\s+rank(?:ing|ed)?\s+(?:on|in|well|high)\b|\bisn'?t\s+rank(?:ing|ed)\b|\bgoogle\s+(?:search\s+)?rankings?\b|\borganic\s+(?:traffic|rankings?)\b|\bkeyword\s+(?:research|rankings?|analysis)\b/i;
...
function isSearchMarketingPitch(t) {
  ...
  if (RANKING_TOPIC.test(t) && (SALES_SIGNAL.test(t) || AUDIT_OPENER.test(t) || WEB_SELLING.test(t))) return true;
...
function isBlockedText(t, opts) {
  ...
  if (!sellsSearch && (isSeoSolicitation(t, opts) || isLeadGenPitch(t) || isSpecWebsitePitch(t) || isSpecAiAgentPitch(t))) return true;
