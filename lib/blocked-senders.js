...
function isGeneratedNameToken(raw) {
  ...
  return GEN_NAME_BASES.some((base) => {
    ...
    if (/^[A-Z]/.test(m[2])) return false; // "JohnMark"
    return !GEN_NAME_REAL_ENDINGS.has(m[2]);
  });
}
...
function hasGeneratedName(t) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const row = lines[i].match(/^(?:full\s*|first\s*|last\s*|your\s*|contact\s*)?name\s*:\s*(\S+)$/i);
    if (row && isGeneratedNameToken(row[1])) return true;
    if (LABEL.test(lines[i]) && i + 1 < lines.length && isGeneratedNameToken(lines[i + 1])) return true;
  }
