...
//
// Fifteen rules, all deliberately DIFFERENT from the heuristic spam scoring used
...
function isGeneratedNameToken(raw) {
  ...
  return GEN_NAME_BASES.some((base) => {
    ...
    const m = t.slice(base.length).match(/^(\d{0,3})([a-z]{3,5}|[A-Z][a-z]{1,4})$/);
    if (!m) return false;
...
function hasGeneratedName(t) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i] || '';
    const row = line.match(/^(?:full\s*|first\s*|last\s*|your\s*|contact\s*)?name\s*:\s*(\S+)$/i);
    if (row && isGeneratedNameToken(row[1])) return true;
    if (LABEL.test(line) && i + 1 < lines.length && isGeneratedNameToken(lines[i + 1])) return true;
  }
  ...
  return !!(subj && isGeneratedNameToken(subj[1]));
}

function isGeneratedNameRussianContact(t) {
  if (!RU_EMAIL.test(t) && !RU_PHONE.test(t)) return false;
